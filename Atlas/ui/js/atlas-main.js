        ...
        function showLoggedOut() {
            ...
            document.getElementById('authArea').innerHTML = `
                <button class="login-btn" onclick="document.getElementById('loginModal').style.display='flex'">Giriş Yap</button>
            `;
            initSessions();
        }
        ...
        function renderSessions() {
            ...
            select.innerHTML = sessions.map(s => `
                <option value="${s.id}" ${s.id === activeSessionId ? 'selected' : ''}>${s.title}</option>
            `).join('');
            saveSessions();
        }
